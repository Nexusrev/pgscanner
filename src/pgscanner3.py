...
# Create a console object to use rich print capabilities
console = Console()
...
log_file_path = os.path.join(base_dir, 'scan_log.log')

# Setup logging with RichHandler for better terminal output
logging.basicConfig(
    level="INFO",
    format="%(message)s",
    datefmt="[%X]",
...
def find_config(config, search_root, progress):
...
def search_configs(search_root, configs):
    ...
    with Progress() as progress:
        ...
        with Pool(processes=len(configs)) as pool:
            results = pool.starmap(find_config, [(config, search_root, progress) for config in configs])
